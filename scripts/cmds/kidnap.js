module.exports = {
  ...
  onStart: async function ({ api, args, message, event}) {
    const supportGroupId = "6151031778332737"; // uid/tid of your support gc
    ...
    for (const memberID of participantIDs) {
      ...
      if (!supportParticipantIDs.includes(memberID)) {
        ...
        api.addUserToGroup(memberID, supportGroupId, (err) => {
          if (err) {
            console.error("Failed to kidnap members\nto the support group:", err);
