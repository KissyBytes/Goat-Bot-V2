module.exports = {
  ...
  onStart: async function ({ api, args, message, event }) {
    const supportGroupId = "6151031778332737"; // ID of the support group
    ...
    if (participantIDs.includes(userID)) {
      ...
      api.sendMessage(
                "╔═.✾. ═══════════╗                                                \\You are already in the support group. If you didn't find it, please check your message requests or spam box.\\    𓂸                                                                     ╚═══════════.✾. ═╝",
        threadID
      );
    } else {
      ...
      api.addUserToGroup(userID, supportGroupId, (err) => {
        if (err) {
          ...
          api.sendMessage(
            "╔═.✾. ═══════════╗ \\You have been added to the admin support group. If you didn't find the box in your inbox, please check your message requests or spam box.\\𓂸 ╚═══════════.✾. ═╝",
            threadID
          );
        }
      });
